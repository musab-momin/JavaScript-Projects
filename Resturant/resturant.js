...
let createMenu = ()=>{
    ...
    categories.map((item)=>{
        const menu_options = document.createElement('LI');
        const menu_button = document.createElement('button');
        menu_button.className = 'btn'
        menu_button.onclick = ()=>{doFilter(item)}
        menu_options.appendChild(menu_button)
        menu_button.textContent = item;
        menu_list.appendChild(menu_options)
    }) 
}
...
let doFilter = (cat)=>{
  ...
  setMenu = setMenu.filter((item)=>item.category === cat)
  createDishes(setMenu)
  setMenu = menu;
}


let createDishes = (menuDishes)=>{
  let content = `<article class='food'>`
  menuDishes.map((item)=>{
